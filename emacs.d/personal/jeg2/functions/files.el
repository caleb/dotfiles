;;;###autoload
(defun personal/read-file-to-string (path)
...
;;;###autoload
(defun personal/find-subpath-in-path (subpath path)
  ...
  (let ((match (concat (file-name-as-directory path) subpath)))
    (if (file-exists-p match)
        match
      (unless (string= path "/")
        (personal/find-subpath-in-path
         subpath
         (file-name-directory (substring path 0 -1)))))))

;;;###autoload
(defun personal/find-in-path (subpath)
