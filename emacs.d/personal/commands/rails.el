...
;;;###autoload
(defun personal/rails-console ()
  "Invoke inf-ruby with Rails environment loaded."
  (interactive)
  (let ((config (personal/find-in-path "config/environment.rb")))
    (if config
...
;;;###autoload
(defun personal/rails-dbconsole ()
  "Open a SQL shell using the settings from config/database.yml."
  (interactive)
  (let ((config (personal/find-in-path "config/database.yml")))
    (if config
        (let* ((env     (personal/read-rails-database-config config))
               (adapter (car env))
